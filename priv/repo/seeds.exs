...
#
#     ContactsPhoenixElm.Repo.insert!(%ContactsPhoenixElm.SomeModel{})
#
# We recommend using the bang functions (`insert!`, `update!`
# and so on) as they will fail if something goes wrong.

defmodule ContactsPhoenixElm.Seeds do
  ...
  def generate_email(name) do
    [_, domain] = Regex.split(~r{@}, Faker.Internet.email)
    "#{String.downcase(name)}@#{domain}"
  end
end

alias ContactsPhoenixElm.{Repo, Contact, Seeds}
...
for index <- 1..99 do
  ...
  params = %{
    ...
    email: Seeds.generate_email(first_name),
